...
data <- data %>% mutate(
  TC_10risk_sd = TC_10risk_cal/sd(TC_10risk_cal),
  gail_10risk_sd = gail_10risk_cal/sd(gail_10risk_cal),
...
# write.csv(train_IDs, file.path(config$data$derived, "training_ids_mixedeth.csv"), row.names = FALSE)
train_IDs <- read.csv(file.path(config$data$derived, "training_ids_mixedeth.csv"))$x
...
dt <- data.frame(model=c("Tyrer-Cuzick", "Model only", "Model with PRS", 
                 ...
                 AUC=c(NA,
                       AUC_CI(df=test_data, score="TC_10risk_cal", status = "TEU_BrCa_status", dp=2),
                       AUC_CI(df=test_data, score="TC_prs_10risk", status = "TEU_BrCa_status", dp=2),
                       NA,
                       AUC_CI(df=test_data, score="gail_10risk_cal", status = "TEU_BrCa_status", dp=2),
                       AUC_CI(df=test_data, score="gail_prs_10risk", status = "TEU_BrCa_status", dp=2)),
...
ft <- flextable(dt) %>%
  ...
  set_header_labels(model="Model", AUC="AUC (95% CI)",
                    NRI_overall="Overall", 
  ...
  align(align = "center", part = "all", j=c('AUC', 'NRI_overall', 'NRI_case', 'NRI_control')) %>%
  bg(i= ~ is.na(AUC), bg="lightgrey", part="body") %>%
  merge_at(i=1, j=1:5) %>%
  merge_at(i=4, j=1:5) %>%
...
cal_plot <- function(data, var="TC_10risk", title, xlim=c(0.01,0.07), ylim=c(0.01,0.07)){
...
TC_cal <- cal_plot(data=data, var="TC_10risk",
                   title="Tyrer-Cuzick model")
gail_cal <- cal_plot(data=data, var="gail_10risk",
                     title="Gail model") +
  theme(legend.position="none")
TC_cal2 <- cal_plot(data=data, var="TC_10risk_cal",
                   title="Calibrated Tyrer-Cuzick model")
gail_cal2 <- cal_plot(data=data, var="gail_10risk_cal",
                     title="Calibrated Gail model") +
  theme(legend.position="none")


# 2. Save the legend
#+++++++++++++++++++++++
legend <- get_legend(TC_cal)
# 3. Remove the legend from the box plot
#+++++++++++++++++++++++
TC_cal <- TC_cal + theme(legend.position="none")

gridExtra::grid.arrange(TC_cal, gail_cal, legend, TC_cal2, gail_cal2,
                        ncol=3, widths=c(3.5, 3.5, 0.8),
                        top=paste0("Before and after calibration plots of 10-year observed vs predicted risk by predicted risk decile of each model in the whole analysis population (N=", nrow(data), ")"))
...
# New approach uses pROC, Xiaonan's code
# https://gitlab.ndph.ox.ac.uk/jenniferco/prsondiabetes/-/blob/Riskscore/Scripts/Stats_Analysis/RMarkdown/RiskScore_Primary.Rmd#auc



roc_plot <- function(data, modelname, prsname, plotname,
                     varlist=c("gail_10risk_cal", "PRS_bc_std", "gail_prs_10risk")){
  
  AUC_CIs <- sapply(varlist, function(x) AUC_CI(df=data, score=x, status = "TEU_BrCa_status", dp=2))
  
  AUC_labels <- c(
    paste0(modelname, " 10-yr risk: ", AUC_CIs[1]),
    paste0(prsname, " PRS: ", AUC_CIs[2]),
    paste0(modelname, " & PRS 10-yr risk: ", AUC_CIs[3])
  )
  
  long <- data %>%
    select(ID, TEU_BrCa_status, all_of(varlist)) %>%
    pivot_longer(!c("ID","TEU_BrCa_status"), names_to = "Score", values_to = "Value") %>%
    mutate(Score=factor(Score,levels = varlist))
  
  
  plot <- ggplot(long, aes(d=TEU_BrCa_status, m=Value, color=Score)) +
    geom_roc(n.cuts = 0, linealpha = 1, size=1) +
    style_roc() +
    geom_abline(aes(slope=1, intercept=0), linetype="dashed") +
    scale_color_manual(name="AUC (95% CI)", labels =  AUC_labels, values = c("#F8766D","#00BA38", "#6193FF")) +
    labs(title = paste0(plotname), 
         caption = "Dashed line is y=x") +
...
# Tyrer-Cuzick
TC_roc <- roc_plot(test_data, modelname="TC", prsname="Genomics plc", plotname="Tyrer-Cuzick",
         varlist=c("TC_10risk_cal", "PRS_bc_std", "TC_prs_10risk"))

# Gail
gail_roc <- roc_plot(test_data, modelname="Gail", prsname="Genomics plc", plotname="Gail",
         varlist=c("gail_10risk_cal", "PRS_bc_std", "gail_prs_10risk"))

gridExtra::grid.arrange(TC_roc, gail_roc,
                        ncol=2,
                        top=paste0("ROC curves for 10-yr risk predictions in test data (N=", nrow(test_data), ")"))
...
nri_tab <- function(p, population, model_old, model_new) {
  ...
  flextable(p) %>%
    ...
    vline(j=c(2, 4)) %>%
    fix_border_issues(part="all") %>%
    footnote(i=1, j=1, value=as_paragraph("Cases defined as individuals diagnosed with breast cancer within 10 years. Controls defined as individuals who were still at risk of breast cancer by 10 years of follow-up. Individuals censored before 10 years are not displayed."), 
           ref_symbols="a", part="header")
}
...
part1 <- tableby(PRS_bc_std_q ~ 
                   ...
                   # Family history
                   TC_MotherBrCa + 
                   TC_NSisters + TC_SiblingBrCa + TC_FatherBrCa,
...
tc_varlist <- c("FSF_AgeMenopause", "FSF_AgeMenarche", "Height", "BMI", 
                ...
                # "FSF_HRT_timesince", "TC_MotherBrCa", "TC_NSisters", "TC_SiblingBrCa", "TC_FatherBrCa")
...
dt <- data.frame(model=c("Tyrer-Cuzick", "Model only", "Model with PRS", "Integrated PRS",
                 ...
                 AUC=c(NA,
                       AUC_CI(df=test_data, score="TC_10risk_cal", status = "TEU_BrCa_status", dp=2),
                       AUC_CI(df=test_data, score="TC_prs313_10risk", status = "TEU_BrCa_status", dp=2),
                       AUC_CI(df=test_data, score="TC_10risk_Mavaddat2019_cal", status = "TEU_BrCa_status", dp=2),
                       NA,
                       AUC_CI(df=test_data, score="gail_10risk_cal", status = "TEU_BrCa_status", dp=2),
                       AUC_CI(df=test_data, score="gail_prs313_10risk", status = "TEU_BrCa_status", dp=2)),
