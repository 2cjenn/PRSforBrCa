...
export_svg(DiagrammeR::grViz(glue(gv))) %>% 
  charToRaw %>% 
  rsvg %>%
  png::writePNG(here::here(file.path(config$outputs$figures$final, "SuppFigureS1.png")))
...
tiff(file.path(config$outputs$figures$final,"SuppFigureS3.tiff"), units="in", width=8, height=5, res=300)
...
tiff(file.path(config$outputs$figures$final,"SuppFigureS4.tiff"), units="in", width=8, height=5, res=300)
...
cal_plot <- function(data, var="TC_10risk", title, xlim=c(0,0.1), ylim=c(0,0.1)){
  ...
  plot <- ggplot(calibration, aes(predicted, observed)) +
    ...
    labs(x="Predicted risk",
         y="Observed risk",
         title=title) +
    annotate("text", x=0.07, y = 0.015, size=10/.pt,
  ...
  ggsave(file.path(config$outputs$figures$interim, paste0(var, "_calibration_plot10.jpg")), 
...
#+++++++++++++++++++++++
# Figure 1
tiff(file.path(config$outputs$figures$final,"Figure1.tiff"), units="in", width=8, height=10, res=300)
gridExtra::grid.arrange(TC_train$plot + theme(legend.position="none"), 
                        ...
                        ncol=3, widths=c(3.5, 3.5, 1),
                        top = grid::grid.text("Figure 1", x = 0, hjust = 0))
dev.off()
...
td_roc_plot <- function(data, modelname, prsname, plotname,
  ...
  tROC_df$Model <- factor(c(rep(paste0(modelname, " 10-yr risk"), times=dim(tROC[[1]][["TP"]])[1]),
                     rep(prsname, times=dim(tROC[[2]][["TP"]])[1]),
                     rep(paste0(modelname, " & PRS_BC 10-yr risk"), times=dim(tROC[[3]][["TP"]])[1])),
                     levels=c(paste0(modelname, " 10-yr risk"),
                              prsname,
                              paste0(modelname, " & PRS_BC 10-yr risk")))
  ...
  ggsave(file.path(config$outputs$figures$interim, paste0("rocplot_", modelname, ".jpg")),
...
# Tyrer-Cuzick
TC_roc <- td_roc_plot(test_data, modelname="Tyrer-Cuzick", prsname="PRS_BC", plotname="Tyrer-Cuzick",
         varlist=c("TC_10risk_cal", "PRS_bc_std", "TC_prs_10risk"))

# Gail
gail_roc <- td_roc_plot(test_data, modelname="Gail", prsname="PRS_BC", plotname="Gail",
         varlist=c("gail_10risk_cal", "PRS_bc_std", "gail_prs_10risk"))

# Figure 2
tiff(file.path(config$outputs$figures$final,"Figure2.tiff"), units="in", width=10, height=5, res=300)
gridExtra::grid.arrange(TC_roc, gail_roc,
                        ncol=2,
                        top = grid::grid.text("Figure 2", x = 0, hjust = 0))
dev.off()
...
dca_tc
ggsave(file.path(config$outputs$figures$interim, "TC_DCA.png"), 
       scale=0.7, width=25, height=15, units="cm")
...
dca_gail
ggsave(file.path(config$outputs$figures$interim, "gail_DCA.png"), 
       scale=0.7, width=25, height=15, units="cm")

# Figure 3
tiff(file.path(config$outputs$figures$final,"Figure3.tiff"), units="in", width=10, height=5, res=300)
gridExtra::grid.arrange(dca_tc, dca_gail, ncol=2,
                        top = grid::grid.text("Figure 3", x = 0, hjust = 0))
dev.off()
...
ia_tc
ggsave(file.path(config$outputs$figures$interim, "TC_NIA.png"), 
       scale=0.7, width=25, height=15, units="cm")
...
ia_gail
ggsave(file.path(config$outputs$figures$interim, "gail_NIA.png"), 
       scale=0.7, width=25, height=15, units="cm")

gridExtra::grid.arrange(ia_tc, ia_gail, ncol=2)
...
# Tyrer-Cuzick
facet_tc <- test_data %>% select(TC_10risk_cal, TC_prs_10risk, TEU_BrCa_status) %>% 
  pivot_longer(cols=c(TC_10risk_cal, TC_prs_10risk)) 

density_data_tc <- facet_tc %>%
  mutate(
...
density_TC <- ggplot(density_data_tc, 
...
# Gail
facet_gail <- test_data %>% select(gail_10risk_cal, gail_prs_10risk, TEU_BrCa_status) %>% 
  pivot_longer(cols=c(gail_10risk_cal, gail_prs_10risk)) 

density_data_gail <- facet_gail %>%
  mutate(
    name = ifelse(TEU_BrCa_status == 0, paste0(name, "_control"), paste0(name, "_case")),
    Model = factor(name, 
...
density_gail <- ggplot(density_data_gail, 
       aes(x=value, fill=Model)) + 
  geom_density(alpha=0.4, position="identity") +
  theme(legend.position = c(0.8, 0.8)) +
  xlim(0, 0.2) +
...
tiff(file.path(config$outputs$figures$final,"SuppFigureS5.tiff"), units="in", width=8, height=10, res=300)
gridExtra::grid.arrange(density_TC, density_gail, nrow=2)
dev.off()
